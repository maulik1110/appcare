...
const HomeProduct = ({ data, title, maxneeded, locateTo }) => {
  ...
  return (
    <div className="flex flex-col flex-wrap md:my-5 w-full ">
      <h1 className="text-center md:text-5xl text-3xl my-5 font-semibold">
        {title}
      </h1>
      <div className="container flex flex-wrap justify-between md:w-full ">
        {limiteddata.map((item, index) => (
          <div
            key={index}
            className="md:w-[24%] w-[48%] hover:scale-[1.01] transition-all duration-150 md:px-10 px-2 flex flex-col md:items-center justify-start rounded-lg shadow-lg"
          >
            <img
              className=" m-2 h-32 w-full rounded-lg"
              src={item.img}
              alt=""
            />
            <h1 className="md:text-lg text-[3vw]  font-semibold my-4">
            ...
            <div className="flex gap-2 items-center my-3">
              ...
              <h1 className="price line-through opacity-80">
                {item.cutprice}/-
              </h1>
            </div>
        ...
        <button onClick={redirectTo} className="px-4 py-2 bg-red-700 rounded-md my-5 text-white mx-auto">
          Shop Now
        </button>
      </div>
    </div>
